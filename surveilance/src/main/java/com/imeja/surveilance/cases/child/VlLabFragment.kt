...
import com.imeja.surveilance.databinding.FragmentVlLabBinding
 ...
 * Use the [VlLabFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class VlLabFragment : Fragment() {
    ...
    private fun handleDataClick(currentCase: String) {
        ...
        FormatterClass().saveSharedPref(
            "title", "VL Laboratory Examination", requireContext()
        )
        val intent = Intent(requireContext(), AddCaseActivity::class.java)
        intent.putExtra(
            QUESTIONNAIRE_FILE_PATH_KEY, "vl-case-lab-information.json"
        )
    ...
    fun parseFromAssets(context: Context): List<OutputGroup> {
        ...
        try {
            val jsonContent =
                context.assets.open("vl-case-lab-information.json").bufferedReader()
                    .use { it.readText() }
    ...
    private fun showLocalOrRegionalLab() {
        ...
        labButton.setOnClickListener {
            dialog.dismiss()
            FormatterClass().saveSharedPref(
                "questionnaire", "measles-lab-results.json", requireContext()
            )
            FormatterClass().saveSharedPref(
                "title", "Measles Lab Results", requireContext()
            )
            val intent = Intent(requireContext(), AddCaseActivity::class.java)
            intent.putExtra(
                QUESTIONNAIRE_FILE_PATH_KEY, "measles-lab-results.json"
            )
            startActivity(intent)
        }
        ...
        regionalLabButton.setOnClickListener {
            dialog.dismiss()
            FormatterClass().saveSharedPref(
                "questionnaire", "measles-lab-reg-results.json", requireContext()
            )
            FormatterClass().saveSharedPref(
                "title", "Regional Measles Lab Results", requireContext()
            )
            val intent = Intent(requireContext(), AddCaseActivity::class.java)
            intent.putExtra(
                QUESTIONNAIRE_FILE_PATH_KEY, "measles-lab-reg-results.json"
            )
            startActivity(intent)
        }
    ...
    companion object {
         ...
         * @return A new instance of fragment VlLabFragment.
         */
        // TODO: Rename and change types and number of parameters
        @JvmStatic
        fun newInstance(param1: String, param2: String) = VlLabFragment().apply {
